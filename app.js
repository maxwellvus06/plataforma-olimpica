...
// =====================================================================
// 🔥 CAMADA FIREBASE/FIRESTORE + STORAGE — mantém o visual e as features antigas
// =====================================================================
// Esta camada troca a base local/Apps Script por Firestore + Storage,
// preservando as funções, telas e IDs originais do projeto.

const AVANCE_FIRESTORE_MAP = {
...
let AVANCE_FIRESTORE_PRONTO = false;
let AVANCE_SYNC_EM_ANDAMENTO = false;

// Impede que o database.js recarregue dados antigos no localStorage.
function garantirCadastrosBasicos() {
    // No modo Firebase, não semeamos dados locais.
    // O Firestore é a fonte da verdade.
}

function campoTimestampFirestore() {
...
function docIdSeguro(valor) {
    ...
    if (base) return base.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 120) || String(Date.now());
...
function normalizarUsuarioFirestore(id, u = {}) {
    return {
        id: id || u.id || novoId(),
        login: String(u.login || u.username || "").trim().toLowerCase(),
        senha: String(u.senha || u.password || ""),
        nivel: u.nivel || u.role || "Aluno",
        nome: u.nome || u.fullname || u.name || u.login || u.username || "Usuário",
        email: u.email || "",
        telefone: u.telefone || "",
...
function prepararParaFirestore(chave, item = {}) {
    const limpo = { ...item };
    delete limpo.__ref;

    if (chave === "app_usuarios") {
        limpo.login = String(limpo.login || limpo.username || "").toLowerCase();
        limpo.senha = String(limpo.senha || limpo.password || "");
        limpo.nome = limpo.nome || limpo.fullname || limpo.login;
        limpo.nivel = limpo.nivel || limpo.role || "Aluno";
        // Campos duplicados para compatibilidade com a versão nova que você já testou.
        limpo.username = limpo.login;
        limpo.password = limpo.senha;
        limpo.fullname = limpo.nome;
        limpo.role = limpo.nivel;
        limpo.vinculo = limpo.vinculoId || limpo.vinculo || "";
    }

    if (chave === "app_premiados") {
        limpo.id = limpo.id || hashTextoSimples(`${limpo.aluno}|${limpo.escola}|${limpo.olimpiada}|${limpo.serie}`);
    }

    limpo.updatedAt = campoTimestampFirestore();
    return limpo;
}

async function carregarColecaoFirestore(chave) {
    initFirebase();
    const nomeColecao = AVANCE_FIRESTORE_MAP[chave];
    if (!nomeColecao || !firebaseFirestore) return [];

    const snap = await firebaseFirestore.collection(nomeColecao).get();
    return snap.docs.map(docSnap => {
        const data = docSnap.data() || {};
        if (chave === "app_usuarios") return normalizarUsuarioFirestore(docSnap.id, data);
        if (chave === "app_premiados") return { id: docSnap.id, ...data };
...
async function carregarBaseFirestoreInicial() {
    ...
    if (!firebaseFirestore) {
        console.warn("Firestore não disponível. Verifique os scripts Firebase no index.html.");
        return [];
    }

    try {
        AVANCE_FIRESTORE_PRONTO = false;
        const chaves = Object.keys(AVANCE_FIRESTORE_MAP);
        const listas = await Promise.all(chaves.map(chave => carregarColecaoFirestore(chave).catch(err => {
            console.warn(`Falha ao carregar ${chave}`, err);
            return [];
        })));

        chaves.forEach((chave, idx) => {
            localStorage.setItem(chave, JSON.stringify(listas[idx] || []));
        });

        AVANCE_FIRESTORE_PRONTO = true;
        return getStorage("app_usuarios");
    } catch (erro) {
        console.error("Erro ao carregar base Firestore", erro);
        AVANCE_FIRESTORE_PRONTO = true;
        return getStorage("app_usuarios");
    }
}
...
async function salvarColecaoFirestoreCompleta(chave, lista) {
    initFirebase();
    const nomeColecao = AVANCE_FIRESTORE_MAP[chave];
    if (!nomeColecao || !firebaseFirestore || !Array.isArray(lista)) return;
    if (AVANCE_SYNC_EM_ANDAMENTO) return;

    AVANCE_SYNC_EM_ANDAMENTO = true;
    try {
        const colRef = firebaseFirestore.collection(nomeColecao);
        const snap = await colRef.get();
        const idsNovos = new Set();

        const batch = firebaseFirestore.batch();
        lista.forEach(itemOriginal => {
            const item = prepararParaFirestore(chave, itemOriginal);
            const id = docIdSeguro(item.id || item.login || item.nome || item.titulo || hashTextoSimples(JSON.stringify(item)));
            idsNovos.add(id);
            batch.set(colRef.doc(id), { ...item, id }, { merge: true });
        });

        snap.docs.forEach(docSnap => {
            if (!idsNovos.has(docSnap.id)) batch.delete(docSnap.ref);
        });

        await batch.commit();
    } catch (erro) {
        console.warn(`Falha ao sincronizar ${chave} no Firestore`, erro);
    } finally {
        AVANCE_SYNC_EM_ANDAMENTO = false;
    }
}

function salvarUsuariosFirebase(usuarios) {
    return salvarColecaoFirestoreCompleta("app_usuarios", usuarios);
}

// Reescreve setStorage para espelhar qualquer CRUD antigo no Firestore.
function setStorage(chave, valor) {
    localStorage.setItem(chave, JSON.stringify(valor));
    if (AVANCE_FIRESTORE_PRONTO && AVANCE_FIRESTORE_MAP[chave]) {
        salvarColecaoFirestoreCompleta(chave, valor).catch(erro => console.warn("Sync Firestore falhou", erro));
    }
}

// Reescreve salvarPremiados para mandar resultados ao Firestore.
function salvarPremiados() {
    setStorage("app_premiados", dadosTrabalho);
}

// ==================== PLATAFORMA: Firebase Storage + Firestore ====================
async function carregarMateriaisPlataforma() {
    const materiais = await carregarColecaoFirestore("app_plataforma").catch(() => getStorage("app_plataforma"));
    localStorage.setItem("app_plataforma", JSON.stringify(materiais || []));
    return (materiais || []).sort((a, b) => Number(b.criadoEm || 0) - Number(a.criadoEm || 0));
}

async function enviarArquivoParaGoogleDrive(arquivo) {
    // Mantém o nome da função antiga para não quebrar o restante do sistema,
    // mas agora o arquivo vai para o Firebase Storage.
    initFirebase();
    if (!firebaseStorage) throw new Error("Firebase Storage não está disponível.");

    const nomeLimpo = String(arquivo.name || "arquivo")
        .normalize("NFD").replace(/[\u0300-\u036f]/g, "")
        .replace(/[^a-zA-Z0-9._-]/g, "_");
    const storagePath = `plataforma/${Date.now()}_${nomeLimpo}`;
    const refArquivo = firebaseStorage.ref(storagePath);
    await refArquivo.put(arquivo, { contentType: arquivo.type || "application/octet-stream" });
    const fileUrl = await refArquivo.getDownloadURL();
    return { success: true, fileUrl, fileId: storagePath, fileName: arquivo.name, storagePath };
}

async function excluirArquivoGoogleDrive(fileId) {
    // Mantém o nome antigo, mas remove do Firebase Storage.
    initFirebase();
    if (!fileId || !firebaseStorage) return { success: true };
    try { await firebaseStorage.ref(fileId).delete(); }
    catch (erro) { console.warn("Não foi possível apagar arquivo no Storage", erro); }
    return { success: true };
}
...
async function salvarNovoMaterial(event) {
    ...
    if (!firebaseFirestore) return alert("Firestore não carregou. Confira as regras e scripts do Firebase.");

    try {
        if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin mr-2"></i>Publicando...'; }
        const material = {
            titulo, descricao, area, tipo,
            url: tipo === "video" || tipo === "link" ? url : "",
            criadoPor: usuarioLogado?.nome || "Sistema",
            criadoPorId: usuarioLogado?.id || "",
            criadoEm: Date.now(),
        ...
        if (tipo === "arquivo") {
            ...
            material.driveFileId = upload.storagePath; // compatibilidade visual antiga
            material.nomeArquivo = upload.fileName || arquivo.name;
            material.tamanhoBytes = arquivo.size;
        }

        const docRef = await firebaseFirestore.collection("materiais").add({ ...material, createdAt: campoTimestampFirestore() });
        const cache = getStorage("app_plataforma");
        cache.unshift({ id: docRef.id, ...material });
        localStorage.setItem("app_plataforma", JSON.stringify(cache.slice(0, 200)));

        document.getElementById("formAddMaterial").reset();
        ajustarCamposFormMaterial();
        await renderizarPlataformaEnsino();
        alert("Material publicado com sucesso no Firebase Storage/Firestore.");
    } catch (erro) {
        console.error("Erro ao publicar material:", erro);
        alert(`Erro ao publicar material.\n\n${erro.message || erro}`);
    } finally {
...
async function excluirMaterial(id) {
    ...
    try {
        let material = getStorage("app_plataforma").find(m => m.id === id) || null;
        if (firebaseFirestore) {
            const snap = await firebaseFirestore.collection("materiais").doc(id).get();
            if (snap.exists) material = { id: snap.id, ...snap.data() };
        }

        if (material?.storagePath || material?.driveFileId) await excluirArquivoGoogleDrive(material.storagePath || material.driveFileId);
        if (firebaseFirestore) await firebaseFirestore.collection("materiais").doc(id).delete();

        const materiaisLocais = getStorage("app_plataforma").filter(m => m.id !== id);
        localStorage.setItem("app_plataforma", JSON.stringify(materiaisLocais));
        await renderizarPlataformaEnsino();
        alert("Material removido da plataforma e do Firebase.");
    } catch (erro) {
        console.error("Erro ao apagar material:", erro);
        alert(`Erro ao apagar material.\n\n${erro.message || erro}`);
    }
...
// ==================== MONITORIA: anexos no Firebase Storage ====================
async function enviarArquivoMonitoria(inputEl) {
    if (!inputEl || !inputEl.files || !inputEl.files.length) return;
    if (!salaMoniAtual || !usuarioLogado) return alert("Entre em uma sala antes de enviar arquivo.");

    const arquivo = inputEl.files[0];
    const tamanhoMb = arquivo.size / (1024 * 1024);
    if (tamanhoMb > LIMITE_ANEXO_MONITORIA_MB) return alert(`Arquivo muito grande. Para o chat da monitoria, use até ${LIMITE_ANEXO_MONITORIA_MB} MB.`);

    const statusAnterior = document.getElementById("statusChamadaMonitoria")?.textContent;
    setStatusChamadaMonitoria("Enviando anexo para o Firebase Storage...", "text-amber-400");
    ...
    try {
        ...
        await firebaseDB.ref(`monitoria/${salaMoniAtual}/mensagens`).push({
            ...
            autorNivel: usuarioLogado.nivel,
            texto: ehImagem ? "Imagem enviada" : "Arquivo enviado",
            arquivoUrl: upload.fileUrl,
            driveFileId: upload.storagePath || "",
            nomeArquivo: upload.fileName || arquivo.name,
...
document.addEventListener("DOMContentLoaded", () => {
    aplicarTemaAvance();
    document.getElementById("btnThemeToggleLogin")?.addEventListener("click", alternarTemaAvance);
    document.getElementById("btnThemeToggleHeader")?.addEventListener("click", alternarTemaAvance);
});

// Reexpõe funções sobrescritas no window para os onclicks do HTML antigo.
window.salvarNovoMaterial = salvarNovoMaterial;
window.excluirMaterial = excluirMaterial;
window.enviarArquivoMonitoria = enviarArquivoMonitoria;
